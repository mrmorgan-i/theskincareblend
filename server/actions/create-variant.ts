    ...
    try {
        if(editMode && id) {
            const editVariant = await db
                    ...
                    return {success: `Variant ${productType} updated successfully`}
        } 
        if(!editMode) {
            const newVariant = await db
                                ...
                                revalidatePath('/dashboard/products')
                                return {success: `Variant ${productType} created successfully`}
        }
    } catch (error) {
        return {error: "An error occurred while creating the variant"}
    }
})